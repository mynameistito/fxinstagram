...
The preview uses stage `dev_fxinstagram` and profile `default`; Alchemy authentication may still be required. The Worker receives `PUBLIC_ORIGIN` from its deployed URL and the configured `ALLOWED_MEDIA_HOSTS`, `METADATA_CACHE_TTL_SECONDS`, and `METADATA_TIMEOUT_MS`. Provider secrets, if later needed, belong in the platform secret mechanism, never tracked files. Plan 005 currently deploys only the native Worker and Alchemy state; it does not provision durable cache storage.
